...
describe("API Endpoints", () => {
  ...
  describe("404 Handling", () => {
    ...
    test("GET non-existent entity returns proper error format", async () => {
      ...
      const response = await fetch(`${BASE_URL}/api/entities/nonexistent-id-12345`)
      expect(response.status).toBe(404)
  ...
  describe("Custom Endpoint Validation", () => {
    test("POST /api/liabilities/:id/record-payment without amount returns validation error", async () => {
      ...
      const response = await fetch(`${BASE_URL}/api/liabilities/test-id/record-payment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    test("POST /api/liabilities/:id/record-payment with invalid liability returns not found", async () => {
      ...
      const response = await fetch(`${BASE_URL}/api/liabilities/nonexistent-id/record-payment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          paymentDate: "2024-01-01",
    ...
    test("POST /api/hems-requests/:id/approve with negative amount returns validation error", async () => {
      ...
      const response = await fetch(`${BASE_URL}/api/hems-requests/test-id/approve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          approvedAmount: "-100",
  ...
  describe("Critical Workflow: Liability Payment Recording", () => {
    test("Complete workflow: Create liability → Record payment → Verify balance updated → Verify expense entry created", async () => {
      ...
      const verifyLiabilityResponse = await fetch(`${BASE_URL}/api/liabilities/${liability.id}`)
      ...
      const paymentsResponse = await fetch(`${BASE_URL}/api/liabilities/${liability.id}/payments`)
      ...
      const paymentEntry = accountingEntries.find(
        (e: Record<string, unknown>) => e.id === paymentResult.accountingEntry.id,
      )
    ...
    test("Liability payment with INCOME allocation creates income expense entry", async () => {
      ...
      const entry = entries.find((e: Record<string, unknown>) => e.id === result.accountingEntry.id)
  ...
  describe("Critical Workflow: HEMS Request Approval", () => {
    test("Complete workflow: Create HEMS request → Approve → Verify status change", async () => {
      ...
      const approveResponse = await fetch(`${BASE_URL}/api/hems-requests/${request.id}/approve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const verifyResponse = await fetch(`${BASE_URL}/api/hems-requests/${request.id}`)
    ...
    test("Complete workflow: Create HEMS request → Deny → Verify status and reason", async () => {
      ...
      const denyResponse = await fetch(`${BASE_URL}/api/hems-requests/${request.id}/deny`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
