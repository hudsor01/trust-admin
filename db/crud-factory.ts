...
>(table: T, options: CrudOptions = {}) {
  ...
  return {
    ...
    async getAll(
      ...
      if (!pagination || (!pagination.limit && !pagination.includeTotalCount)) {
        if (filterValue && filterColumn) {
          ...
          if (!column) {
            throw new Error(`Filter column '${filterColumn}' not found in table`)
          }
      ...
      if (includeTotalCount) {
        if (filterValue && filterColumn) {
          ...
          // biome-ignore lint/suspicious/noExplicitAny: Drizzle generic table type requires assertion
          const countResult = await db.select({ count: sql<number>`count(*)` }).from(table as any)
          totalCount = countResult[0] ? Number(countResult[0].count) : 0
        }
      }
    ...
    async create(data: Insert): Promise<Select> {
      ...
      const hasId = "id" in dataRecord && typeof dataRecord.id === "string"

      const values = {
        ...data,
        id: hasId ? (dataRecord.id as string) : generateId(),
        ...(hasUpdatedAt && { updatedAt: new Date().toISOString() }),
      } as typeof data & { id: string; updatedAt?: string }
