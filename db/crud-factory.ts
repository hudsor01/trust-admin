...
/**
 * Type helper for insert input - omits id (database IDENTITY generates it)
 */
...
>(table: T, options: CrudOptions = {}) {
    ...
    return {
        ...
        /**
         * Get all records as a simple array (never paginated)
         *
         * This is a convenience method that always returns Select[],
         * eliminating the need for type guards like:
         *   const result = await crud.getAll(entityId)
         *   return Array.isArray(result) ? result : result.data
         */
        async getAllArray(filterValue?: number): Promise<Select[]> {
            const result = await this.getAll(filterValue)
            return Array.isArray(result) ? result : result.data
        },
         ...
         */
        async getById(id: number): Promise<Select | undefined> {
            if (!columns.id) {
         ...
         */
        async delete(id: number): Promise<Select | undefined> {
            if (!columns.id) {
                throw new Error("Table does not have an 'id' column")
            }
...
> = {
    ...
    getAllArray: (filterValue?: number) => Promise<Select[]>
    getById: (id: number) => Promise<Select | undefined>
    create: (data: Insert) => Promise<Select>
    update: (id: number, data: Partial<Insert>) => Promise<Select | undefined>
    delete: (id: number) => Promise<Select | undefined>
}
