...
function shouldSkipColumn(headerText: string, cell: HTMLTableCellElement): boolean {
...
export function exportTableToCsv(table: HTMLTableElement, filename: string) {
  ...
  bodyRows.forEach((row) => {
    ...
    const values = columnIndexes.map((index) => normalizeText(cells[index]?.textContent))
...
export function exportTablesInContainer(container: HTMLElement, baseName: string) {
