 ...
 */
export function calculateTrusteeFees(
    ...
    if (input.trustAssetValue && input.trustAssetValue > 0) {
        ...
        const annualFee =
            input.trustAssetValue * (schedule.annualAssetPercent / 100)
...
export function generateFeeSummary(result: FeeCalculationResult): string {
    ...
    if (result.breakdown.incomeFee) {
        lines.push(
            `Income/Property Management Fee: ${formatCurrency(result.incomeFee)}`,
        )
    ...
    if (result.breakdown.hourlyFee) {
        lines.push(
            `Extraordinary Services: ${formatCurrency(result.hourlyFee)}`,
        )
