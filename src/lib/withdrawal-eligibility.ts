...
export function calculateWithdrawalEligibility(
  ...
  const hasWithdrawalRights = !!(beneficiary.withdrawalAge1 && beneficiary.withdrawalPct1)
...
export function formatEligibility(e: WithdrawalEligibility): string {
  ...
  if (e.age1) {
    if (e.age1.isEligible) {
      ...
      lines.push(`  Age ${e.age1.targetAge}: ${e.age1.daysUntilEligible} days until eligible`)
      lines.push(`    Will be able to withdraw: $${e.age1.amountAvailable.toLocaleString()}`)
    }
  }
  ...
  if (e.age2) {
    if (e.age2.isEligible) {
      ...
      lines.push(`  Age ${e.age2.targetAge}: ${e.age2.daysUntilEligible} days until eligible`)
      lines.push(`    Will be able to withdraw: $${e.age2.amountAvailable.toLocaleString()}`)
    }
  }
