...
export function validateProductionEnvironment(env: Env): void {
  ...
  if (!env.BETTER_AUTH_SECRET) {
    warnings.push("BETTER_AUTH_SECRET not set - sessions may not persist across restarts")
  }

  if (!env.RESEND_API_KEY) {
    warnings.push("RESEND_API_KEY not set - magic link emails will not be sent")
  }

  if (!env.FRONTEND_URL || !env.API_URL) {
    warnings.push("FRONTEND_URL and/or API_URL not set - may cause CORS issues")
  }

  if (!env.TRUSTED_ORIGINS) {
    warnings.push("TRUSTED_ORIGINS not set - authentication may fail in production")
  }
  ...
  if (!env.SENTRY_DSN) {
    warnings.push("SENTRY_DSN not set - error reporting disabled")
  }
