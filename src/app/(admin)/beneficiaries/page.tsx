...
import { asDistributionStandard, asPaymentMethod } from "@/lib/type-utils"
...
export default function BeneficiariesPage() {
  ...
  const { data: beneficiariesRaw = [], isLoading: beneficiariesLoading } =
    trpc.beneficiary.list.useQuery({ entityId: selectedEntity }, { enabled: !!selectedEntity })
  ...
  // Load distributions for each beneficiary using tRPC
  useEffect(() => {
    const loadDistributions = async () => {
      const withDistributions = await Promise.all(
        beneficiariesRaw.map(async (b) => {
          try {
            const full = await utils.beneficiary.byId.fetch(b.id)
            if (full) {
  ...
  const recordDistribution = async () => {
    ...
    try {
      ...
      const updated = await utils.beneficiary.byId.fetch(selectedBeneficiary.id)
      if (updated) {
    ...
    },
    {
      ...
      render: (b) => (
        <EditableSelectCell
          ...
          onSave={async (val) => {
            await updateBeneficiary(b.id, { distributionStandard: asDistributionStandard(val) })
          }}
        />
      ),
      sortable: true,
...
function BeneficiaryDialogContent({
  ...
  updateBeneficiary: (id: string, data: Partial<Beneficiary>) => Promise<unknown>
