  ...
  },
  {
    ...
    render: (account) => (
      <EditableSelectCell
        ...
        onSave={async (val) => {
          await updateBankAccount(account.id, { status: asRecordStatus(val) })
        }}
      />
    ),
  },
  {
    ...
    render: (account) => (
      <EditableSelectCell
        ...
        variants={STATUS_VARIANTS}
        onSave={async (val) => {
          await updateBankAccount(account.id, { transferStatus: asTransferStatus(val) })
        }}
      />
    ),
  },
  ...
  },
  {
    ...
    render: (account) => (
      <EditableSelectCell
        ...
        variants={STATUS_VARIANTS}
        onSave={async (val) => {
          await updateInvestmentAccount(account.id, { status: asRecordStatus(val) })
        }}
      />
    ),
  },
  {
    ...
    render: (account) => (
      <EditableSelectCell
        ...
        variants={STATUS_VARIANTS}
        onSave={async (val) => {
          await updateInvestmentAccount(account.id, { transferStatus: asTransferStatus(val) })
        }}
      />
    ),
  },
...
export default function AccountsPage() {
  ...
  const { data: investmentAccounts = [] } = trpc.investmentAccount.list.useQuery(
    { entityId: selectedEntity! },
    { enabled: queryEnabled },
  )
  const createInvestmentAccountMutation = trpc.investmentAccount.create.useMutation({
    onSuccess: () => utils.investmentAccount.list.invalidate(),
  })
  const updateInvestmentAccountMutation = trpc.investmentAccount.update.useMutation({
    onSuccess: () => utils.investmentAccount.list.invalidate(),
  })
  const deleteInvestmentAccountMutation = trpc.investmentAccount.delete.useMutation({
    onSuccess: () => utils.investmentAccount.list.invalidate(),
  })
  ...
  return (
    <div className="space-y-6">
      ...
      <ResourceDialog
        ...
        title={bankForm.isEditing ? "Edit Bank Account" : "Add Bank Account"}
      ...
      <ResourceDialog
        ...
        title={investmentForm.isEditing ? "Edit Investment Account" : "Add Investment Account"}
