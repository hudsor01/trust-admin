...
interface FirearmRowDetailProps {
    firearm: Firearm
    /**
     * Called when the user clicks "Update Form 5 Status". The parent
     * (FirearmsClient) owns the NfaStatusDialog so the same dialog can be
     * triggered from the table action column without expanding the row.
     */
    onUpdateNfaStatus?: (firearm: Firearm) => void
}

export function FirearmRowDetail({
    firearm,
    onUpdateNfaStatus,
}: FirearmRowDetailProps) {
            ...
            {firearm.isNfa && (
                <section className="space-y-3">
                    ...
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                        ...
                        <dt className="text-muted-foreground">ATF Form Type</dt>
                        <dd>
                            {firearm.atfFormType
                                ? (ATF_FORM_TYPE_LABELS[firearm.atfFormType] ??
                                  firearm.atfFormType)
                                : '—'}
                        </dd>
                        ...
                        <dt className="text-muted-foreground">NFRTR Serial</dt>
                    ...
                    <div className="flex items-center gap-3 pt-2">
                        <Badge
                            variant={STATUS_VARIANTS[nfaStatus] ?? 'secondary'}
                        >
                            {NFA_TRANSFER_STATUS_LABELS[nfaStatus] ?? nfaStatus}
                        </Badge>
                        {onUpdateNfaStatus && (
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => onUpdateNfaStatus(firearm)}
                            >
                            ...
                            {detail.valuations.map((v) => (
                                ...
                                >
                                    <span className="text-muted-foreground">
                                        {v.valuationDate} · {v.valuationType}
                                    </span>
