  ...
  if (editing) {
    return (
      <Select
        ...
        defaultOpen
      >
        <SelectTrigger className="h-7 text-sm w-35">
          <SelectValue />
...
export function EditableNumberCell({
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const bounded = numVal !== null ? Math.max(min, Math.min(max, numVal)) : null
  ...
  return (
    <div
      ...
      className="cursor-pointer rounded px-2 py-1 -mx-2 -my-1 hover:bg-muted/50 min-h-7 flex items-center"
    >
      <span className={cn("text-sm font-medium", value === null && "text-muted-foreground")}>
