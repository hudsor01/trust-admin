            ...
            if (localeDecimalSeparator === ',') {
                ...
                if (!hasDecimalSeparator && commaIndex !== -1) {
                    ...
                    const looksLikeThousands =
                        commaIndex <= 3 && afterComma.length >= 3
                    if (
                        !looksLikeThousands &&
                        afterComma.length <= 2 &&
                        /^\d*$/.test(afterComma)
...
function MaskInput(props: MaskInputProps) {
    ...
    const tokenCount = React.useMemo(() => {
        if (
            !maskPattern ||
            CURRENCY_PERCENTAGE_SYMBOLS.test(maskPattern.pattern)
        )
            return undefined
    ...
    const onInputValidate = React.useCallback(
        (unmaskedValue: string) => {
            if (onValidate && maskPattern?.validate) {
                const isValid = maskPattern.validate(
                    unmaskedValue,
                    validationOpts,
                )
        ...
        (event: React.ChangeEvent<InputElement>) => {
            ...
            if (maskPattern) {
                ...
                if (inputRef.current && newValue !== inputValue) {
                    ...
                    if (
                        isCurrencyMask({ mask, pattern: maskPattern.pattern })
                    ) {
                        if (mask === 'currency') {
                            const currencyAtEnd = isCurrencyAtEnd(transformOpts)
                            if (!currencyAtEnd) {
